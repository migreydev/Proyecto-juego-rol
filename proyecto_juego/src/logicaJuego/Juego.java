...
public class Juego {
	...
	private Map<Coordenada, Jugador> tablero;
	...
	public Juego(PlayerType[] tipos) {
        ...
        for (int i = 0; i < tipos.length; i++) {
            Coordenada coordenada = new Coordenada(i + 1, i + 1); 

            Jugador jugador = new Jugador(tipos[i]);
	...
	public Map<Coordenada, Jugador> getTablero() {
		return tablero;
	}


	public void setTablero(Map<Coordenada, Jugador> tablero) {
	...
	public String moverJugador(char direccion) throws JuegoException, JugadorException {
	    ...
	    // Verificar si hay algo en la nueva casilla
	...
	private String luchar(String resultado, Jugador jugador, Coordenada coordDestino, Jugador elemento) throws JugadorException {
	...
	private void cambiaJugadorAPosicion(Coordenada coordDestino) {
		...
		Jugador player=(Jugador)this.tablero.get(cor);
		tablero.remove(cor);
	...
	private void crearGemas() {
		...
		while(cont<Constantes.NUM_GEMAS) {
			...
			if(!tablero.containsKey(coord)) {
				this.tablero.put(coord, (Jugador) e);
				cont+=1;
			}
		}
	}
	...
	public void setCoordenadaJugadores(List<Coordenada> coordenadaJugadores) {
		this.coordenadaJugadores = coordenadaJugadores;
	}
